...
public class MessageDecoder() : LengthFieldBasedFrameDecoder(short.MaxValue, 0, 4, 0, 4)
{
    ...
    private object Decode(Packet packet)
    {
        return packet.TypedPacketCase switch
        {
            ...
            Packet.TypedPacketOneofCase.Text => new TextMessage(packet.Text.Text, packet.Text.Location),
            ...
            Packet.TypedPacketOneofCase.Say => new CreatureSayMessage(packet.Say.Id, packet.Say.Text, packet.Say.ViewName, packet.Say.Cache),
            ...
            Packet.TypedPacketOneofCase.GainExp => packet.GainExp.KungFu ? new KungFuGainExpMessage(packet.GainExp.Level, packet.GainExp.Name, packet.GainExp.Attack) : new GainExpMessage(packet.GainExp.Name),
            ...
            Packet.TypedPacketOneofCase.ShowTeleport => TeleportSnapshot.FromPacket(packet.ShowTeleport),
