...
public partial class Monster : AbstractEntity, INpcMessageHandler
{
    ...
    public void Initialize(NpcSnapshot snapshot)
    {
        ...
        {
            case NpcState.Move:
                CreateMover(snapshot.Direction, (float)snapshot.StateSnapshot.ElapsedMillis / 1000);
                _animationPlayer.PlayMove(snapshot.Direction, snapshot.StateSnapshot.ElapsedMillis);
                break;
            case NpcState.Idle:
                _animationPlayer.PlayIdle(snapshot.Direction, snapshot.StateSnapshot.ElapsedMillis);
                break;
            case NpcState.Attack:
                _animationPlayer.PlayAttack(snapshot.Direction, snapshot.StateSnapshot.ElapsedMillis);
                break;
            case NpcState.Die:
                _animationPlayer.PlayDie(snapshot.Direction, snapshot.StateSnapshot.ElapsedMillis);
                break;
            case NpcState.Hurt:
                _animationPlayer.PlayHurt(snapshot.Direction, snapshot.StateSnapshot.ElapsedMillis);
                break;
        }
        Visible = true;
    ...
    public void Move(MoveMessage message)
    {
        ...
        if (_animationPlayer.CurrentAnimation.Contains(NpcState.Move.ToString()))
        {
            Logger.Debug("Received move {} for {}, current coordinate {}.", message, message.Id, Coordinate);
        }
    ...
    public static Monster Create()
    {
        ...
        return scene.Instantiate<Monster>();
    }
}