...
public partial class Game : Node2D
{
    ...
    private void OnInventoryItemDropped(int slot)
    {
        ...
        var entity = _entityManager.FindFirst<IEntity>(e => e.HasMouseHover());
        if (entity != null)
        {
    ...
    {
        _map.Load(message.MapFile, message.ResourceName, message.Coordinate);
    ...
    private void OnCharacterTeleported(TeleportMessage teleportMessage)
    {
        ...
        _map.Load(teleportMessage.MapFile, teleportMessage.ResourceName, teleportMessage.Coordinate);
        _character.Teleported(teleportMessage.Coordinate);
        Logger.Debug("Character teleported.");
    }


    private IEnumerable<GroundItem> FilterItems()
    {
        return _entityManager.Where(e => e is GroundItem).OfType<GroundItem>();
    }
